...
long InvalidIt = -3; // used for removing infreq items
...
bool mine_induced = false;
bool mine_ordered = false;
...
void parse_args(int argc, char **argv)
{
   ...
   if (argc < 2)
     ...
     while ((c=getopt(argc,argv,"a:bfi:IlOop:s:S:uz:"))!=-1){
       switch(c){
       case 'a':
   ...
   if (mine_induced && !count_unique){
      cout << "incuded and multiple counts not supported yet!\n";
      exit(-1);
   }
}
...
list<Eqclass *> * get_F2()
{
  ...
  for (i=0; i < DCB->NumF1; i++){
   ...
   te = tt.Stop();
   stats.add(DCB->NumF1 * DCB->NumF1, 0, te);

   BranchIt = DCB->NumF1+100; //make branchit more than all other items
   return F2list;
}

inline bool notfrequent (Eqnode &n, int iter){
  //cout << "IN FREQ " << n.sup << endl;
   //if (mine_induced){
   //   if (n.isup >= MINSUPPORT) return false;
   //}
   //else{
...
void check_ins(idlist *l1, idlist *l2, Eqnode *ins, 
                int st1, int st2, int en1, int en2, bool f2){
   static idnode *n1, *n2;
   ...
   while(st2 < en2){
      ...
      case sup: 
         ...
         if (depth >= 1 && n1->path_equal(*n2)){
            //if (f2) depth = n2->depth-n1->depth;
            //else depth = n2->depth;
            //if (n2->depth == n1->depth+1 && n1->induced)
            //   induced = true;
            if (depth == 1 && n1->induced) induced = true;
            else induced = false;
            if (!f2) depth = n2->depth;
            ...
            if (en1-st1 > 1 || use_fullpath){
               ...
               ins->add_list(n2->cid, n1->parscope, 
                             n1->itscope.lb, n2->itscope, depth, induced);
            }
            else{
               //ins->tlist.push_back(idnode(n2->cid,n2->itscope));
               ins->add_list(n2->cid,n2->itscope, depth, induced);
            }
            if (!count_unique) ins->sup++;
            found_flg = true;
            if (induced) induced_flg = true;
         }
...
//                               n1->itscope.lb, n2->itscope, n2->depth);
//             }
//             else{
//                //outs->tlist.push_back(idnode(n2->cid,n2->itscope));
//                outs->add_list(n2->cid,n2->itscope, n2->depth);
//             }
//             if (!count_unique) outs->sup++;
//             found_flg = true;
//             if (n2->depth == rdepth) induced_flg = true;
//          }
//          //next2 = false;
//          break;
      case after:
         if (mine_ordered && cmpval == after) break;
         
         //n1 is before n2. Check if n1.par is subset of or equal to n2.par
         if (n1->path_equal(*n2)){
            //if (n1->depth == n1depth && n2->depth == n2depth &&
            ...
            if (en1 - st1 > 1 || use_fullpath){
               ...
               //                            n1->itscope.lb, n2->itscope));
               outs->add_list(n2->cid, n1->parscope, 
                              n1->itscope.lb, n2->itscope, n2->depth, induced);
            }
            else{
               //outs->tlist.push_back(idnode(n2->cid,n2->itscope));
               outs->add_list(n2->cid,n2->itscope,n2->depth, induced);
            }
            if (!count_unique) outs->sup++;
            found_flg = true;
            if (induced) induced_flg = true;
   ...
   if (induced_flg) outs->isup++;
}


void get_intersect(idlist *l1, idlist *l2, Eqnode *ins, Eqnode *outs,
                   int n1depth, int n2depth, bool f2=false)
{
   static idnode *n1, *n2;
   ...
   while (i1 < l1->size() && i2 < l2->size()){
      ...
      if (n1->cid < n2->cid) i1++;
         ...
         if (ins) check_ins(l1, l2, ins, i1, i2, e1, e2, f2);
...
{   
   if (mine_ordered) return true;
   
   //only have to check subtrees on rightmost path, with their
   //immediate siblings to the left, and make sure code is smaller
   // or equal
   bool first;
   int i, parent=0, sibling;
   int scnt = 0;

   //base condition for stopping recursion
   if (tpos == 0){
      return true;
   }
...
//    cout << "CC " << tpos << " " << sibling << " " << parent << " -- ";
//    for (i=0; i < cand.size(); ++i)
//       if (cand[i] != -1){
//          cout << " " <<  DCB->FreqIdx[cand[i]];
//       }
//      else cout << " " << cand[i];
//    cout << endl;
   ...
   bool res = true;
   if (sibling == -1){
      res = check_canonical(cand, parent);
   }
   else{
      //comapre the two subtrees rooted at tpos and sibling
      int j;
      //int it1, it2;
      for(i=tpos, j=sibling; i < cand.size() && j < tpos; i++, j++){
         //it1 = DCB->FreqIdx[cand[i]];
         //it2 = DCB->FreqIdx[cand[j]];
         //it1=cand[i];
         //it2=cand[j];
         //cout << "XX " << i << " " << j << " " 
         //     << it1 << " " << it2 << endl;
         
         if (cand[i] > cand[j]) break;
         else if (cand[i] < cand[j]){
            res = false;
            break;
         }
      }
      if (res) res = check_canonical(cand, parent);
   }
   //cout << "RES = " << res << endl;
   return res;
}
...
Eqnode *test_node(int iter, Eqclass *eq, int val, int pos)
{
   ...
   //check if this patten is in canonical form
   //res = check_canonical(cand, cand.size()-1);
   res= true;

   //if not canonical, will be pruned
   if (!res) return eqn;
...
void enumerate_freq(Eqclass *eq, int iter)
{
   ...
   Eqnode *ins, *outs;
   int ni_depth, nj_depth; //required depth for induced mining
   
   if (prune_type == noprune) eq->sort_nodes(); //doesn't work with pruning

   //cout << endl << "FX " << *eq << "------ " << endl;

   for (ni = eq->nlist().begin(); ni != eq->nlist().end(); ++ni){
      eq->get_scope((*ni)->pos, ni_depth);
      neq = new Eqclass;
      neq->set_prefix(eq->prefix(),*(*ni));

      bool res;
      res = ((!mine_induced || (*ni)->isup >= MINSUPPORT) && 
             check_canonical(neq->prefix(), neq->prefix().size()-1)); 
      if (!res){
         delete neq;
         continue;
      }
      
      stats.incrlarge(iter-2);
      if (output){
         //cout << "INDUCED ";
         neq->print_prefix();
         cout << *(*ni);
      ...
      for (nj = eq->nlist().begin(); nj != eq->nlist().end(); ++nj){ 
         ...
         eq->get_scope((*nj)->pos, nj_depth);
         //cout << "prefix ";
         //eq->print_prefix();
         //cout << " -- " << flush << *(*nj) << " ,,, " <<  sss << endl;
         ...
         if (outs){
            ...
            //cout << "OUTS " << *outs;
            if (notfrequent(*outs, iter)) delete outs;
            else{
               neq->add_node(outs);
               //if (!mine_induced || outs->isup >= MINSUPPORT)
         ...
         if (ins){
            // cout << "INS " << *ins;
            stats.incrcand(iter-1);
            if (notfrequent(*ins, iter)) delete ins;
            else{
               neq->add_node(ins);
               //if (!mine_induced)
               //   stats.incrlarge(iter-1);
      ...
      if (!neq->nlist().empty()){
//          if (output){
//             if (mine_induced) neq->print(true);
//             else neq->print();
//          }
...
void form_f2_lists(Eqclass *eq)
{
   ...
   for (ni=eq->nlist().begin(); ni != eq->nlist().end(); ++ni){
      ...
      get_intersect(l1, l2, ins, outs, 0, 0, true);
...
int main(int argc, char **argv)
{
   ...
   summary << "SLEUTH ";
   if (mine_induced) summary << "INDUCED ";
   else summary << "EMBEDDED ";
   
   if (mine_ordered) summary << "ORDERED ";
   else summary << "UNORDERED ";
