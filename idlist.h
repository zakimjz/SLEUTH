...
public:
   ...
   int depth; //node's depth in the original tree with given cid
   bool induced; //is this node a valid induced node

   idnode(int c=0, int l=0, int u=0, int d=0, bool i=false): 
      cid(c), itscope(l,u), depth(d), induced(i){};

   idnode(int c, ival &it, int d, bool i=false){
      cid = c;
      itscope = it;
      depth = d;
      induced = i;
   ...
   idnode(int c, vector<int> &pscope, int lit, ival &ivl, int d, 
          bool i=false){
      cid = c;
      itscope = ivl;
      parscope = pscope;
