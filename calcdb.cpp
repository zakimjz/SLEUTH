...
void Dbase_Ctrl_Blk::get_valid_trans()
{
      ...
      if (TransAry[i] != InvalidIt){
         if (TransAry[i] != BranchIt){   
            if (FreqMap[TransAry[i]] == -1){
               ...
               if (!mine_induced) TransAry[--j] = InvalidIt;
            }
         }
      }
   }
   ...
   if (mine_induced){
      ...
      for (i=0, newTransSz=0; i < TransSz; i++){
	 if (TransAry[i] == BranchIt || TransAry[i] == InvalidIt) 
	    PvtTransAry[newTransSz] = TransAry[i]; 
	 else PvtTransAry[newTransSz] = FreqMap[TransAry[i]];
	 newTransSz++;
      }
   }
   else{
      ...
      for (i=0,newTransSz=0; i < TransSz; i++){
	 if (TransAry[i] != InvalidIt){
	    if (TransAry[i] == BranchIt) PvtTransAry[newTransSz] = TransAry[i];
	    else PvtTransAry[newTransSz] = FreqMap[TransAry[i]];
	    newTransSz++;
	 }
      }
...
void Dbase_Ctrl_Blk::make_vertical(){
   ...
   for (i=0, pi=0; i < TransSz; i++){
	 ...
	 continue;
      ...
      idnode nn(Cid, pi,pj, depth, true);
